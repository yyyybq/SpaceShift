#!/bin/bash
MODEL="${MODEL:-nyu-visionx/Cambrian-S-7B}"
TASK_NAME="${TASK_NAME:-scene_variation}"
OUTPUT_DIR="/nas2/edwin/lmms-eval/results/cambrians_7b_${TASK_NAME}"
...
$PYTHON -m accelerate.commands.launch --num_processes="${NUM_PROCESSES}" --main_process_port="${MASTER_PORT}" -m lmms_eval \
    ...
    --log_samples --log_samples_suffix "cambrians_7b_${TASK_NAME}" \
