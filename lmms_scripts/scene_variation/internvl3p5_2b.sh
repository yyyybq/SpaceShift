#!/bin/bash
MODEL="${MODEL:-OpenGVLab/InternVL3_5-2B}"
TASK_NAME="${TASK_NAME:-scene_variation}"
OUTPUT_DIR="/nas2/edwin/lmms-eval/results/internvl3p5_2b_${TASK_NAME}"
...
$PYTHON -m accelerate.commands.launch --num_processes="${NUM_PROCESSES}" --main_process_port="${MASTER_PORT}" -m lmms_eval \
    ...
    --log_samples --log_samples_suffix "internvl3p5_2b_${TASK_NAME}" \
